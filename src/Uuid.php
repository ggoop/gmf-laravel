...
class Uuid {
	 ...
	 * @throws Exception
	 */
	protected function __construct($uuid, $space = '-') {
		...
		$this->string = bin2hex(substr($uuid, 0, 4)) . $space .
		bin2hex(substr($uuid, 4, 2)) . $space .
		bin2hex(substr($uuid, 6, 2)) . $space .
		bin2hex(substr($uuid, 8, 2)) . $space .
	...
	public static function generate($ver = 1, $node = null, $ns = null, $space = '-') {
		...
		switch ((int) $ver) {
		case 1:
			return (new static(static::mintTime($node), $space))->string;
		case 2:
		case 3:
			return (new static(static::mintName(static::MD5, $node, $ns), $space))->string;
		case 4:
			return (new static(static::mintRand(), $space))->string;
		case 5:
			return (new static(static::mintName(static::SHA1, $node, $ns), $space))->string;
		}
	}
	/**
	 * 生成ID
	 * @param  string  $format m=月,d=天,y=年,w=周,q=季度,默认=ymdxxxxxx
	 * x结尾为序号补位数
	 * @param  string  $tag    [description]
	 * @param  integer $count  [description]
	 * @return [type]          [description]
	 */
	public static function sequence($format = 'ymdxxxxxx', $tag = '', $count = 1) {
		$start = intval(\date($format));
		if (empty($tag)) {
			$tag = '';
		}
		preg_match("/x+$/", $format, $matchs);
		$key = 'UID.' . $tag . $start;

		if (isset($matchs[0])) {
			$start = $start * pow(10, isset($matchs[0]) ? strlen($matchs[0]) : 0);
		}

		//RDS 模式
		// $maxID = RDS::get($key);
		// if (!$maxID||$maxID<$start) {
		//     $maxID =$start;
		// }
		// $maxID = $maxID + $count;
		// RDS::set($key, $maxID);
		...
		//数据库 模式
		$idObj = Models\Uid::firstOrCreate(['id' => $key]);
		if ($idObj->sn > $start) {
			$maxID = $idObj->sn;
		} else {
			$maxID = $start;
		...
		$maxID = $maxID + $count;
		Models\Uid::where('id', $key)->update(['sn' => $maxID]);
		return $maxID;
	}
	/**
