...
<script>
  export default {
    ...
    methods: {
      create(){
        this.$router.push({ name: 'module', params: { module: 'cbo.mfc.category.edit' }});
      },
      edit(item){
        this.$router.push({ name: 'id', params: { module: 'cbo.mfc.category.edit',id:item.id }});
      },
      remove(){
        ...
        this.$http.delete('cbo/mfc-categories/'+ids).then(response => {
          this.load();
          this.loading--;
