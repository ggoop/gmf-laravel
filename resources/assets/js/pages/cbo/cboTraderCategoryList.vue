<template>
  <md-part>
    ...
    <md-part-body>
      <md-query @select="select" @dblclick="edit" ref="list" md-query-id="gmf.cbo.trader.category.list"></md-query>
...
<script>
  export default {
    ...
    methods: {
      create(){
        this.$router.push({ name: 'module', params: { module: 'cbo.trader.category.edit' }});
      },
      edit(item){
        this.$router.push({ name: 'id', params: { module: 'cbo.trader.category.edit',id:item.id }});
      },
      remove(){
        ...
        this.$http.delete('cbo/trader-categories/'+ids).then(response => {
          this.load();
          this.loading--;
