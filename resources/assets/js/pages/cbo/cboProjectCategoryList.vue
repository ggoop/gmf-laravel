<template>
  <md-part>
    ...
    <md-part-body>
      <md-query @select="select" @dblclick="edit" ref="list" md-query-id="gmf.cbo.project.category.list"></md-query>
...
<script>
  export default {
    ...
    methods: {
      create(){
        this.$router.push({ name: 'module', params: { module: 'cbo.project.category.edit' }});
      },
      edit(item){
        this.$router.push({ name: 'id', params: { module: 'cbo.project.category.edit',id:item.id }});
      },
      remove(){
        ...
        this.$http.delete('cbo/project-categories/'+ids).then(response => {
          this.load();
          this.loading--;
