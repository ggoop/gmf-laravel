<template>
  <md-part>
    ...
    <md-part-body>
      <md-query @select="select" @dblclick="edit" ref="list" md-query-id="gmf.cbo.person.list"></md-query>
...
<script>
  export default {
    ...
    methods: {
      create(){
        this.$router.push({ name: 'module', params: { module: 'cbo.person.edit' }});
      },
      edit(item){
        this.$router.push({ name: 'id', params: { module: 'cbo.person.edit',id:item.id }});
      },
      remove(){
        ...
        this.$http.delete('cbo/persons/'+ids).then(response => {
          this.load();
          this.loading--;
