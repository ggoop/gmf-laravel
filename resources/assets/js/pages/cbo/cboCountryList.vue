<template>
  <md-part>
    ...
    <md-part-body>
      <md-query @select="select" @dblclick="edit" ref="list" md-query-id="gmf.cbo.country.list"></md-query>
...
<script>
  export default {
    ...
    methods: {
      create(){
        this.$router.push({ name: 'module', params: { module: 'cbo.country.edit' }});
      },
      edit(item){
        this.$router.push({ name: 'id', params: { module: 'cbo.country.edit',id:item.id }});
      },
      remove(){
        ...
        this.$http.delete('cbo/countries/'+ids).then(response => {
          this.load();
          this.loading--;
